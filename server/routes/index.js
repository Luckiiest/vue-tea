...
var QcloudSms = require('qcloudsms_js') // 腾讯云短信验证码sdk
...
router.post('/api/payment', function (req, res, next) {
  ...
  formData.addField('returnUrl', 'http://localhost:8080/payment') // 支付成功回调地址，必须为可以直接访问的地址，不能带参数

  const result = alipaySdk.exec('alipay.trade.page.pay', {}, { formData: formData }) // result 为可以跳转到支付链接的 url
  //对接支付宝成功，支付宝方返回的数据
  result.then((resp) => {
    res.send({
      data: {
  ...
  connection.query(`select * from user where tel=${tokenObj.tel}`, function (error, results) {
    ...
    connection.query(`select * from store_order where uId = ${uId} and order_id=${orderId}`, function (err, result) {
      ...
      connection.query(`update store_order set order_status=replace(order_status,'1','2') where id='${id}'`, function (e, r) {
        // 把购物车数据删除
        shopArr.forEach((v) => {
          connection.query(`delete from goods_cart where id='${v}'`, function (er, re) {})
        })
        res.send({
          data: {
            code: 200,
...
router.post('/api/addOrder', function (req, res, next) {
  ...
  function randomNumber() {
    ...
    month = setTimeDataFmt(month)
    day = setTimeDataFmt(day)
    hour = setTimeDataFmt(hour)
    minutes = setTimeDataFmt(minutes)
    seconds = setTimeDataFmt(seconds)
