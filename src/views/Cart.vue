...
=======
  <div class='cart container'>
    <header>
      ...
      <span>购物车</span>
      <span @click="isNavBar"
            v-text='isNavStatus ? "完成" : "编辑"'>编辑</span>
    </header>
    <section v-if="list.length">
      <div class='
            cart-title'>
        <van-checkbox @click="checkAllFn"
                      :value="isCheckedAll"></van-checkbox>
        <span>商品</span>
      </div>
      <ul>
        <li v-for="(item,index) in list"
            :key="index">
          <div class='check'>
            <van-checkbox @click="checkItem(index)"
          ...
          <div class='goods'>
            <div class='goods-title'>
              ...
              <i class='iconfont icon-lajitong'
                 @click="delGoodsFn(item.id)"></i>
            </div>
            <div class='goods-price'>{{item.goods_price}}</div>
            <van-stepper @change="changeNum($event,item)"
    ...
    <section class="guangguang"
             v-else>
      <h1>没有商品信息，<router-link :to='{name: "Home"}'>去首页逛逛吧~</router-link>
      </h1>
    </section>
    <footer v-if="list.length">
      <div class='radio'>
        <van-checkbox :value="isCheckedAll"></van-checkbox>
      </div>
      <div class='total'
           v-show="!isNavStatus">
        <div>
  ...
  methods: {
    ...
    goOrder () {
      ...
      http.$axios({
        ...
        // 这一句，在method中...mutations[("inintOrder")]了，
        // 这个函数在store里面order.js中
